...
# အီမိုဂျီပေါင်း (၆၀၀) ကျော် ပါဝင်သော စာရင်းအပြည့်အစုံ
CALL_EMOJIS = list(
    "😀😃😄😁😆😅😂🤣🥲☺️😊😇🙂🙃😉😌😍🥰😘😗😙😚😋😛😝😜🤪🤨🧐🤓😎🥸🤩🥳😏😒😞😔😟😕🙁☹️😣😖😫😩🥺😢😭😮‍💨😤😠😡🤬🤯😳🥵🥶😱😨😰😥🤗🤔🤭🤫🤥😶😶‍🌫️😐😑😬🙄😯😦😧😮😲🥱😴🤤😪🤒🤕🤢🤮🤧🥴😵😵‍💫🤠🤖🎃😺😸😹😻😼😽🙀😾"
    "👋🤚🖐️✋🖖👌🤌🤏✌️🤞🫰🤟🤘🤙👈👉👆🖕👇☝️👍👎✊👊🤛🤜👏🙌👐🤲🤝🙏✍️💅💪🦾🦿🦵🦶👂🦻👃🧠🫀🫁🦷🦴👀👁️👅👄💋🩸"
...
async def ban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not reply:
        await update.message.reply_text("⚠️ Reply to user message to ban.")
        return
    try:
        await context.bot.ban_chat_member(chat_id=chat_id, user_id=reply.from_user.id)
        await update.message.reply_text(f"🔨 Banned user {reply.from_user.first_name}.")
...
async def mute_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply = update.message.reply_to_message
    if not reply:
        await update.message.reply_text("⚠️ Reply to user message to mute.")
        return
    try:
...
async def unmute_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply = update.message.reply_to_message
    if not reply:
        await update.message.reply_text("⚠️ Reply to user message to unmute.")
        return
    try:
        ...
        await context.bot.restrict_chat_member(chat_id=chat_id, user_id=reply.from_user.id, permissions=permissions)
        await update.message.reply_text(f"🔊 Unmuted {reply.from_user.first_name}.")
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}")
...
async def call_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "delete_msg":
        ...
        flags = list(LANGUAGES.keys())
        rows, row = [], []
        for flag in flags:
            row.append(InlineKeyboardButton(flag, callback_data=f"lang_{flag}"))
            if len(row) == 5:
                rows.append(row)
                row = []
        if row:
            rows.append(row)
        rows.append([InlineKeyboardButton(texts["btn_back"], callback_data="back_settings")])
        await query.message.edit_text(texts["select_lang"], reply_markup=InlineKeyboardMarkup(rows), parse_mode='Markdown')
    elif data.startswith("lang_"):
        selected_flag = data.split("_")[1]
        if selected_flag in LANGUAGES:
            settings['language'] = selected_flag
        await call_settings(update, context)
    elif data == "back_settings":
        await call_settings(update, context)
    elif data == "call_count":
        counts = [3, 4, 5, 6, 7, 10, 15]
        curr = settings['call_count']
        settings['call_count'] = counts[(counts.index(curr) + 1) % len(counts)] if curr in counts else 5
        await call_settings(update, context)
    elif data == "call_who":
        whos = ["all", "admin", "owner"]
        curr = settings['who_can_call']
        settings['who_can_call'] = whos[(whos.index(curr) + 1) % len(whos)] if curr in whos else "all"
        await call_settings(update, context)
    elif data == "call_mode":
        settings['call_mode'] = "link" if settings['call_mode'] == "emoji" else "emoji"
        await call_settings(update, context)
...
async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    for member in update.message.new_chat_members:
        ...
        chat_id = update.effective_chat.id
        settings = get_group_settings(chat_id)
        caption = settings["welcome_text"].replace("{name}", member.first_name)
        settings_call = get_call_settings(chat_id)
        texts = LANGUAGES.get(settin
